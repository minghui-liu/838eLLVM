#lang racket
(provide current-objs asm-interp asm-interp/io)
...
(define (asm-interp/io a input)
  (define t.ll   (make-temporary-file "llvm~a.ll"))
  (define t.o   (path-replace-extension t.ll #".o"))
  (define t.so  (path-replace-extension t.ll #".so"))
  (define t.in  (path-replace-extension t.ll #".in"))
  (define t.out (path-replace-extension t.ll #".out"))
  ...
  (define entry
    (get-ffi-obj "entry" libt.so (_fun _pointer _-> _int64)))
...
(struct exn:llc exn:fail:user ())
(define llc-msg
   "assembly error")

(define (llc:error msg)
  (raise (exn:llc (format "~a\n\n~a" llc-msg msg)
                   (current-continuation-marks))))

;; run llc on t.ll to create t.o
(define (llc t.ll t.o)
  (define err-port (open-output-string))
  (unless (parameterize ((current-error-port err-port))
            (system (format "llc -march=x86-64 -filetype=obj -o ~a ~a " t.o t.ll)))
    (llc:error (get-output-string err-port))))
